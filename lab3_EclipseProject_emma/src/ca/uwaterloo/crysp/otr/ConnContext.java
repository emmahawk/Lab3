...
public class ConnContext implements OTRContext {
    ...
    public String messageSending(String message, OTRTLV[] tlvs, int fragPolicy,
    	...
    	System.out.println("MsgState: "+msgState.getCurState());
        ...
        if (otrmessage.getType() == OTRMessage.MSG_QUERY) {
        	System.out.println("Setting up the OTR Query");
    ...
    void setSmpTrust(OTRCallbacks callback, boolean trusted){
         ...
         * OTR buttons. */
    	callback.writeFingerprints();
    	System.out.println("checking smptrust");
    }
    ...
    void initRespondSmp(String question, 
    	...
    	{
    		System.out.println("Message state: "+msgState.getCurState());
    	    if (msgState.getCurState()!=MsgState.ST_ENCRYPTED) {
    	    	System.out.println("Msg is not yet encrypted.");
    	    	return;
    	    }
    	    ...
    	    if(initiating){
    	    	...
    	    	//this user's fingerprint is copied into the combined_bf 
    	    	System.arraycopy(our_fp, 0, combined_buf, 1, 20);
    	    	System.out.println("our_fp: "+our_fp);
    	    	//then the active/responder fingerprint is copied into the combined buffer
    	    	System.arraycopy(this.activeFingerprint.fingerPrint, 0, combined_buf, 21, 20);
    	    	System.out.println("active_fp: "+this.activeFingerprint.fingerPrint);
    	    }else{
    	    ...
    	    if(initiating){
    	    	...
    	    	System.out.println("step1 done with secret: "+secret);
    	    }else{
    	    	System.out.println("not initiating. made it here");
    	    	//smstate.x2 is null
    	    	smpmsg = SM.step2b(smstate, combined_secret, prov);
    	    	System.out.println("step2 done with secret: "+secret);
    ...
    public StringTLV messageReceiving(String inMessage, OTRCallbacks callback) throws OTRException {
        ...
        case OTRMessage.MSG_QUERY:
        	...
        	System.out.println("Starting AKE");
            /* Start AKE */
        	try{
	            auth.startAKE(null);
        ...
        case OTRMessage.MSG_DATA:
            ...
            case MsgState.ST_FINISHED:
                ...
                break;
            case MsgState.ST_ENCRYPTED:
            	System.out.println("At least entered ST_ENCRYPTED block in msgreceived");
                byte[] res;
                ...
                try {
                    ...
                    System.out.println("set up res for future use in messRec");
                } catch (OTRException otre) {
                    callback.handleMsgEvent(OTRCallbacks.OTRL_MSGEVENT_RCVDMSG_UNREADABLE, 
                    		this, null);
                    System.out.println("We are quitting out of messageReceived now");
                    return null;
                }
                int end = 0;
                ...
                if (end != res.length&&end+1!=res.length) {
	    		    ...
	    		    if (tlv != null && nextMsg == SM.EXPECT1) {
	    				...
	    				System.out.println("Step2a initiated");
    ...
    QueryMessage defaultQueryMessage(String ourname, int policy) {
        System.out.println("requested default query message");
    	String msg = "?OTR"
                + "v2?"
                + "\n<b>"
                + ourname
...
public class ConnContext implements OTRContext {
    ...
    void initRespondSmp(String question, 
    		String secret, boolean initiating, OTRCallbacks callback) throws OTRException
    	{
    	    if (msgState.getCurState()!=MsgState.ST_ENCRYPTED) 
    	    {
    	    	System.out.println("Response Skipped. Not Encrypted");
    	    }
    	    /*
    	     * Construct the combined secret as a SHA256 hash of:
    	     * Version byte (0x01), Initiator fingerprint (20 bytes),
